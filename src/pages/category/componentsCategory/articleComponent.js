...
// function that gets the articles from "articleCategory" and displays the results
...
return (
  <Container>
      {Array.isArray(articles ) && articles.map((article, index) => (
        <Row
          key={article.uri}
          className={`my-3 ${index !== articles.length - 1 ? " border-bottom" : ""}`}
        >
          <Col>
            {article && (
              <div className="my-5">
